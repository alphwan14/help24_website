...
export function StoryDemo({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      ref={frame}
      className={`relative overflow-hidden rounded-card border border-border bg-card shadow-lift ${className}`}
    >
      {/* ── Frame header ───────────────────────────────────────────────── */}
...
const AskField = forwardRef<HTMLInputElement, AskFieldProps>(function AskField(
  ...
  return (
    <div className="flex h-full flex-col justify-center">
      ...
      <div className="relative flex items-center rounded-button border border-border bg-page focus-within:border-primary focus-within:shadow-[0_0_0_1px_var(--primary)]">
        <input
          id="story-input"
          ref={ref}
          type="text"
...
function RequestCard({ scenario, dropping }: { scenario: Scenario; dropping: boolean }) {
  return (
    <article
      className={`h-full rounded-card border border-border bg-page px-3 py-2.5 ${
...
function Status({ tone, icon, children }: { tone: Tone; icon: string; children: React.ReactNode }) {
  return (
    <p
      className={`flex h-full items-center gap-2 rounded-button bg-page px-3 text-body font-medium ${TONE[tone]}`}
    >
...
function Searching({ label }: { label: string }) {
  return (
    <div className="relative h-full overflow-hidden rounded-button bg-page">
      <span
        ...
        aria-hidden
      />
      <p className="relative flex h-full items-center gap-2 px-3 text-body font-medium text-text-secondary">
...
function OfferRow({ offer, picked = false }: { offer: DemoOffer; picked?: boolean }) {
  return (
    <div
      className={`flex items-center gap-2.5 rounded-card border bg-page px-2.5 py-2 transition-[border-color,box-shadow] duration-300 ${
  ...
  return (
    <ul className="space-y-2" aria-hidden>
      {[0, 1, 2].map((i) => (
        <li
          key={i}
          className="flex items-center gap-2.5 rounded-card border border-border bg-page px-2.5 py-2 opacity-60"
...
function JobProgress({ step, amount }: { step: number; amount: number }) {
  ...
  return (
    <ol className="flex items-start gap-1.5 rounded-card border border-border bg-page px-2.5 py-3">
      {stages.map((s, i) => {
...
function NoSample() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-2.5 rounded-card border border-dashed border-border px-5 text-center">
...
function Prompt() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-2 rounded-card border border-dashed border-border px-4 text-center">
...
function StaticStory({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`overflow-hidden rounded-card border border-border bg-card shadow-lift ${className}`}>
